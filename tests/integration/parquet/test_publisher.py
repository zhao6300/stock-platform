...
def _row() -> tuple[dict[str, object], ...]:
    return (
        {
...
def test_publisher_reuses_and_reads_one_object(tmp_path: Path) -> None:
    store = ImmutableParquetObjectStore(tmp_path)
    publisher = DatasetPublisher(store)
    rows = _row()

    first = publisher.publish(DAILY_BAR_SCHEMA, rows)
    second = publisher.publish(DAILY_BAR_SCHEMA, rows)

    assert first == second
    assert store.read(first).to_pylist() == list(rows)
